...
export default function Dashboard({ user, tasks, onNavigateToTask, onNavigateToPage }: DashboardProps) {
  ...
  const smartMatches = [
    ...(user.motivation === 'Study' ? [{ label: 'Same University', count: 12, icon: <GraduationCap size={24} /> }] : []),
    { label: 'Same Arrival Month', count: 45, icon: <Calendar size={24} /> },
    { label: 'Same Nationality', count: 8, icon: <Globe size={24} /> },
  ];
  ...
  return (
    <div className="space-y-10 w-full mx-auto py-6">
      ...
      <header className="flex justify-between items-center mb-8">
        <div>
          <h1 className="font-display text-3xl font-bold text-n-800">Welcome back, {user.name}</h1>
      ...
      {hasVisa && (
        <section className="bg-g-900 text-white rounded-[2rem] p-8 relative overflow-hidden group shadow-2xl animate-in fade-in slide-in-from-top-4 duration-700">
          <div className="absolute top-0 right-0 w-[500px] h-full bg-white/5 skew-x-[-15deg] translate-x-32 group-hover:translate-x-24 transition-transform duration-1000"></div>
          
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
             <div className="flex-1">
               <div className="flex items-center gap-4 mb-4">
                 <span className="px-3 py-1 bg-g-600 text-white text-[10px] font-black rounded-full uppercase tracking-widest">Pre-arrival Setup</span>
                 <span className="text-white/60 text-xs font-display italic tracking-wide">Save time, reduce stress</span>
               </div>
               ...
               <p className="text-white/60 text-sm max-w-xl leading-relaxed">
                 Don't wait until you land. Set up your <span className="text-white font-bold underline decoration-g-600 decoration-2 underline-offset-4 cursor-pointer">NIF</span>, <span className="text-white font-bold underline decoration-g-600 decoration-2 underline-offset-4 cursor-pointer">Bank Account</span>, and <span className="text-white font-bold underline decoration-g-600 decoration-2 underline-offset-4 cursor-pointer">SIM Card</span> today through our Assisted Service flow.
               </p>
             </div>
             ...
             <div className="flex flex-col items-center gap-5 shrink-0">
                <button 
                  ...
                  className="bg-white text-g-900 font-black px-7 py-3.5 rounded-xl shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2 text-sm group/btn"
                >
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-8 bg-white rounded-[20px] shadow-soft border border-g-100 p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            {nextTask ? (
              <>
                <h2 className="font-display text-2xl font-bold mb-2 text-n-800">{nextTask.title}</h2>
        ...
        <div className="md:col-span-4 bg-white rounded-[20px] shadow-soft border border-g-100 p-8 text-center flex flex-col items-center justify-center">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <h3 className="font-display text-xl font-bold">Smart Matches for You</h3>
      ...
      <section className="mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="bg-g-50/50 border border-g-100 rounded-[2.5rem] p-8 relative overflow-hidden group shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex gap-8 items-center flex-1 relative z-10">
            <div className="w-20 h-20 bg-g-100/50 text-g-600 rounded-[2rem] flex items-center justify-center shadow-inner shrink-0 scale-110 border border-g-100">
              <Home size={40} strokeWidth={1.5} />
            </div>
            <div>
              <h3 className="text-2xl font-bold text-n-800 mb-2">Find a place before you arrive</h3>
              <p className="text-g-800/60 text-base max-w-xl leading-relaxed font-medium">
          ...
          <button 
            ...
            className="bg-g-600 text-white font-bold px-10 py-4 rounded-2xl hover:bg-g-700 transition-all shadow-xl shadow-g-200 shrink-0 flex items-center gap-3 group/btn relative z-10"
          >
            Explore Housing <ArrowRight size={20} className="group-hover/btn:translate-x-1 transition-transform" />
          </button>

          {/* Decorative background elements */}
          <div className="absolute -right-20 -bottom-20 w-64 h-64 bg-g-100/30 rounded-full blur-3xl group-hover:bg-g-100/50 transition-colors" />
          <div className="absolute -left-10 -top-10 w-32 h-32 bg-g-200/20 rounded-full blur-2xl" />
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-6">
          <div className="flex justify-between items-end px-2">
            <h3 className="font-display text-xl font-bold">Current Task List</h3>
            ...
            {tasks.slice(0, 3).map((task) => {
              ...
              return (
                <div 
                  ...
                  className={`bg-white p-5 rounded-2xl border border-g-100 shadow-soft flex items-center gap-5 transition-all cursor-pointer hover:border-primary/20 ${task.status === 'Locked' || isDisabled ? 'opacity-50 grayscale' : ''} ${isDisabled ? 'cursor-not-allowed' : ''}`}
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${
                    task.status === 'Completed' ? 'bg-g-50' : 
                    task.status === 'In Progress' ? 'bg-orange-50 font-bold border-l-4 border-l-accent rounded-l-none' : 
                    'bg-gray-50'
                  }`}>
                    {task.icon === 'Phone' ? <Smartphone size={24} /> : 
                     task.icon === 'FileText' ? <FileText size={24} /> : 
                     task.icon === 'Landmark' ? <Landmark size={24} /> : 
                     <FileEdit size={24} />}
                  </div>
                  <div className="flex-1">
                    <div className="font-bold text-n-800">{task.title}</div>
        ...
        <div className="lg:col-span-5 space-y-6">
           <h3 className="font-display text-xl font-bold px-2">Arrival Timeline</h3>
           <div className="bg-white rounded-[20px] shadow-soft border border-g-100 p-8 relative flex-1 min-h-[300px]">
              ...
              <div className="space-y-12">
                {[
                  ...
                  <div key={idx} className="relative pl-12">
                    ...
                    <div className={`text-sm ${item.status === 'In Progress' ? 'font-bold text-n-800' : 'text-gray-500'}`}>
                      {item.title}
                    </div>
                  </div>
                ))}
