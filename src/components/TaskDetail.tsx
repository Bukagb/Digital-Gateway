...
export default function TaskDetail({ taskId, tasks, onBack, onUpdateStatus }: TaskDetailProps) {
              ...
              <section className="bg-white p-8 rounded-[2.5rem] border border-teal-50 shadow-soft">
                ...
                <div className="mt-8 p-4 bg-orange-50 rounded-2xl flex gap-3">
                  ...
                  <p className="text-xs text-orange-800 leading-normal">
                    <strong>Note:</strong> Some offices now require appointments booked weeks in advance. Check the official portal below.
                  </p>
                </div>
              </section>
              ...
              <section className="bg-white p-8 rounded-[2.5rem] border border-organic shadow-soft">
                ...
                <div className="space-y-4">
                   <div className="p-4 bg-gray-50 rounded-xl">
                      <p className="text-xs font-bold text-gray-400 uppercase mb-1">Scheduling</p>
                      <p className="text-sm font-medium">Wait time can be 2–4 months</p>
                   </div>
                   <div className="p-4 bg-gray-50 rounded-xl">
        ...
        return (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="relative h-64 rounded-[2.5rem] overflow-hidden shadow-2xl">
                <img 
                  src="https://images.unsplash.com/photo-1555854817-ff0b031427a1?auto=format&fit=crop&q=80&w=800" 
                  alt="Student housing" 
                  className="w-full h-full object-cover"
                  referrerPolicy="no-referrer"
                />
            ...
            {/* Trusted Platforms */}
            <section className="bg-white p-8 rounded-[2.5rem] border border-teal-50 shadow-soft">
              <h3 className="text-xl font-bold mb-8">Trusted Platforms</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[
                  { name: 'Uniplaces', url: 'https://www.uniplaces.com/', logo: 'U', color: 'bg-blue-600' },
                  { name: 'Idealista', url: 'https://www.idealista.pt/', logo: 'I', color: 'bg-green-600' },
                  { name: 'Spotahome', url: 'https://www.spotahome.com/', logo: 'S', color: 'bg-orange-600' }
                ].map((platform) => (
                  <a 
                    key={platform.name}
                    href={platform.url}
                    target="_blank"
                    rel="no-referrer"
                    className="flex flex-col items-center p-6 bg-gray-50 rounded-3xl border border-gray-100 hover:scale-105 transition-transform group"
                  >
                    <div className={`w-12 h-12 rounded-2xl ${platform.color} flex items-center justify-center text-white font-black text-xl mb-4 shadow-lg group-hover:rotate-6 transition-transform`}>
                      {platform.logo}
                    </div>
                    <h4 className="font-bold text-lg mb-1">{platform.name}</h4>
                    <span className="text-[10px] text-primary font-bold flex items-center gap-1 uppercase tracking-widest">
                      Visit Site <ExternalLink size={10} />
                    </span>
                  </a>
                ))}
              </div>
              <div className="mt-8 p-4 bg-orange-50 rounded-2xl flex items-center gap-3">
                 <AlertCircle className="text-accent shrink-0" size={20} />
                 <p className="text-xs text-orange-900 leading-normal">
                   <strong>Facebook Groups:</strong> Use with extreme caution. Never pay a deposit before seeing a room in person or through a verified platform.
                 </p>
              </div>
            </section>
    ...
    <div className="max-w-4xl mx-auto py-6 space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-8">
        <div>
          ...
          <p className="text-text-muted text-lg">{task.description}</p>
        </div>
